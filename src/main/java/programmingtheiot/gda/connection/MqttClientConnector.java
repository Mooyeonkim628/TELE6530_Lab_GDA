...
public class MqttClientConnector implements IPubSubClient, MqttCallbackExtended
{
			...
			_Logger.log(Level.SEVERE, "Failed to connect MQTT client to broker.", e);
				if (this.mqttClient != null) {
					try {
						...
						_Logger.log(Level.SEVERE, "Failed to close MQTT client after failed connection.", closeException);
					}
	...
	public boolean disconnectClient()
	{
		try {
			...
			if (this.mqttClient != null) {
				try {
					this.mqttClient.close();
					_Logger.info("MQTT client forcefully closed after failed disconnect.");
				} catch (MqttException closeException) {
					_Logger.log(Level.SEVERE, "Failed to force close MQTT client.", closeException);
				}
				this.mqttClient = null; 
			}
		}
	...
	public boolean publishMessage(ResourceNameEnum topicName, String msg, int qos)
	{
		...
		_Logger.info("Publishing to " + topicName.getResourceName() + " with QoS=" + qos);
